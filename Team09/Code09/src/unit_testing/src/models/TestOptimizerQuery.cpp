...
TEST_CASE("Separate single group of connected clauses - should return single optimizer group") {
	...
	REQUIRE(actual.at(0).getAdjList() == expected);
}
...
TEST_CASE("Separate multiple groups of connected clauses - should return multiple optimizer groups") {
	...
	std::unordered_map<std::string, std::vector<OptimizerClause>> expected1 = {
		{"ifs", {{"ifs", "s1",  0, clause1}}},
		{"s1",  {{"s1",  "ifs", 0, clause1}, {"s1", "pn", 0, clause2}, {"s1", "pn", 0, clause3}, {"s1", "v1", 0, clause5}}},
		{"pn",  {{"pn",  "s1",  0, clause2}, {"pn", "s1", 0, clause3}, {"pn", "v1", 0, clause4}}},
		{"v1",  {{"v1",  "pn",  0, clause4}, {"v1", "s1", 0, clause5}}},
	};

	std::unordered_map<std::string, std::vector<OptimizerClause>> expected2 = {
		{"a", {{"a", "a", 0, clause6}}}
	};
	...
	REQUIRE(actual.at(0).getAdjList() == expected1);
	REQUIRE(actual.at(1).getAdjList() == expected2);
}

...
ModifiesEvaluator::getModifiesByUnderscore(const std::string& LHS, const std::string& RHS, EntityType LHSType) {
	...
	case EntityType::INT:
		// e.g. Modifies("9", _)
		if (!ModifiesStore::getVariablesModifiedByStatement(std::stoi(LHS)).empty())
	...
	case EntityType::STMT:
		...
		queryResult.addColumn(LHS, ModifiesStore::getModifiesStatements());
		break;
	case EntityType::ASSIGN: {
		...
		queryResult.addColumn(LHS, PKBUtil::unorderedSetIntersection(ModifiesStore::getModifiesStatements(), assignStmts));
		break;
	}
	case EntityType::READ:
		// e.g. read r; Modifies(r, _)
		queryResult.addColumn(LHS, PKBUtil::unorderedSetIntersection(ModifiesStore::getModifiesStatements(),
...
ModifiesEvaluator::getModifiesByVariable(const std::string& LHS, const std::string& RHS, EntityType LHSType) {
	...
	std::unordered_set<int> stmtsUsingVar = ModifiesStore::getStatementsModifyingVariable(RHS);
	switch (LHSType) {
	case EntityType::INT:
		// e.g. Modifies("9", "x")
		if (ModifiesStore::getVariablesModifiedByStatement(std::stoi(LHS)).count(RHS))
			queryResult.setBooleanResult(true);
		break;
		/* Not used in iteration 1
	case EntityType::STRING:
	...
	case EntityType::STMT:
		...
		queryResult.addColumn(LHS, ModifiesStore::getStatementsModifyingVariable(RHS));
		break;
	case EntityType::ASSIGN: {
...
ModifiesEvaluator::getModifiesBySynonym(const std::string& LHS, const std::string& RHS, EntityType LHSType) {
	...
	case EntityType::INT:
		// e.g. Modifies("9", v)
		queryResult.addColumn(RHS, ModifiesStore::getVariablesModifiedByStatement(std::stoi(LHS)));
		return queryResult;
		/* Not used in iteration 1
	case EntityType::STRING:
	...
	case EntityType::STMT: {
		...
		std::tie(stmts, vars) = ModifiesStore::getStmtsToModifiedVariable(ModifiesStore::getModifiesStatements());
		break;
	}
	case EntityType::ASSIGN: {
		...
		std::tie(stmts, vars) = ModifiesStore::getStmtsToModifiedVariable(
			PKBUtil::unorderedSetIntersection(ModifiesStore::getModifiesStatements(), assignStmts));
		break;
	}
	case EntityType::READ: {
		// e.g. read r; Modifies(r, v)
		std::tie(stmts, vars) = ModifiesStore::getStmtsToModifiedVariable(EntityStore::getStatementsWithType(EntityType::READ));
		break;
	}
	case EntityType::IF: {
		// e.g. if ifs; Modifies(ifs, v)
		std::tie(stmts, vars) = ModifiesStore::getStmtsToModifiedVariable(EntityStore::getStatementsWithType(EntityType::IF));
		break;
	}
	case EntityType::WHILE: {
		// e.g. while w; Modifies(w, v)
		std::tie(stmts, vars) = ModifiesStore::getStmtsToModifiedVariable(EntityStore::getStatementsWithType(EntityType::WHILE));
		break;
	}
	default:
		return queryResult;

...
QueryClauseResult ProcProcRelationshipEvaluator::getRelationship(RelationRef relationship, const std::string& LHS, const std::string& RHS, EntityType LHSType, EntityType RHSType,
	...
	if (isValidCallsType && LHSType == RHSType && LHS == RHS) { // Edge case of same procedure calling same procedure
		...
		return ProcProcRelationshipEvaluator::getRelationshipByProcedureSynonym(relationship, LHS, RHS, LHSType, RHSType);
	}
...
QueryClauseResult ProcProcRelationshipEvaluator::getTransitiveRelationship(RelationRef relationship, const std::string& LHS, const std::string& RHS, EntityType LHSType, EntityType RHSType,
	bool isBooleanResult) {
	return ProcProcRelationshipEvaluator::getRelationship(relationship, LHS, RHS, LHSType, RHSType, isBooleanResult);
}

QueryClauseResult ProcProcRelationshipEvaluator::getRelationshipByProcedureString(RelationRef relationship, const std::string& LHS, const std::string& RHS, EntityType RHSType) {
	...
	if (RHSType == EntityType::STRING) { // Calls("First", "Second")
		...
		if (PKB::isProcProcFirstEntity(relationship, LHS)) {
			std::unordered_set<std::string> procedures = PKB::getProcProcSecondEntities(relationship, LHS);
...
QueryClauseResult ProcProcRelationshipEvaluator::getRelationshipByProcedureSynonym(RelationRef relationship, const std::string& LHS, const std::string& RHS, EntityType LHSType, EntityType RHSType) {
	QueryClauseResult queryResult;

	if (RHSType == EntityType::STRING) { // Calls(p, "Second")
		if (PKB::isProcProcSecondEntity(relationship, RHS)) {
			std::unordered_set<std::string> procedures = PKB::getProcProcFirstEntities(relationship, RHS);
			queryResult.addColumn(LHS, procedures);
		}
	}
	else if (RHSType == EntityType::PROC) { // Calls(p1, p2)
		auto [firstEntities, secondEntities] = PKB::getAllProcProcRelationshipPairs(relationship);
		queryResult.addColumn(LHS, firstEntities);
		queryResult.addColumn(RHS, secondEntities);
	}
	else if (RHSType == EntityType::WILD) { // Calls(p, _)
		std::unordered_set<std::string> procedures = PKB::getAllProcProcFirstEntities(relationship);
		queryResult.addColumn(LHS, procedures);
	}
...
QueryClauseResult ProcProcRelationshipEvaluator::getRelationshipByUnderscore(RelationRef relationship, const std::string& RHS, EntityType RHSType) {
	QueryClauseResult queryResult;
	...
	if (RHSType == EntityType::STRING) { // Calls(_, "Second")
		...
		std::unordered_set<std::string> procedures = PKB::getAllProcProcSecondEntities(relationship);
		queryResult.addColumn(RHS, procedures);
	}

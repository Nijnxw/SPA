	...
	switch (LHSType) {
		...
		case EntityType::PRINT:
			...
			queryResult.addColumn(LHS,
								  PKBUtil::unorderedSetIntersection(usesStatements, EntityStore::getStatementsWithType(EntityType::PRINT)));
			break;

		case EntityType::IF:
			// e.g. if ifs; Uses(ifs, _)
			queryResult.addColumn(LHS,
								  PKBUtil::unorderedSetIntersection(usesStatements, EntityStore::getStatementsWithType(EntityType::IF)));
			break;

		case EntityType::WHILE:
			// e.g. while w; Uses(w, _)
			queryResult.addColumn(LHS,
								  PKBUtil::unorderedSetIntersection(usesStatements, EntityStore::getStatementsWithType(EntityType::WHILE)));
			break;
		default:
			return queryResult;
	}
...
QueryClauseTable UsesStore::getUsesBySynonym(const std::string& LHS, const std::string& RHS, EntityType LHSType) {
	...
	switch (LHSType) {
		...
		case EntityType::PRINT: {
			...
			std::tie(stmts, vars) = getStmtsToUsedVariable(EntityStore::getStatementsWithType(EntityType::PRINT));
			break;
		}
		case EntityType::IF: {
			// e.g. if ifs; Uses(ifs, v)
			std::tie(stmts, vars) = getStmtsToUsedVariable(EntityStore::getStatementsWithType(EntityType::IF));
			break;
		}
		case EntityType::WHILE: {
			// e.g. while w; Uses(w, v)
			std::tie(stmts, vars) = getStmtsToUsedVariable(EntityStore::getStatementsWithType(EntityType::WHILE));
			break;
			default:
				return queryResult;
		}

...
TEST_CASE("ModifiesStore API") {
	...
	SECTION("Initialization of program variables") {
		...
		SECTION("getStmtToVarByStmts() positive query") {
			...
			std::unordered_set<std::pair<std::string, std::string>, PKBUtils::pairHashFunction> set = 
				PKBUtils::convertVectorTupleToSetPairs(std::get<0>(res), std::get<1>(res));
			std::unordered_set<std::pair<std::string, std::string>, PKBUtils::pairHashFunction> expectedSet =
				PKBUtils::convertVectorTupleToSetPairs(std::get<0>(expectedRes), std::get<1>(expectedRes));
